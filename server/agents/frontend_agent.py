...
1. "file_path": A string with the full proposed file path (e.g., "src/components/LoginButton.tsx").
...
class FrontendAgent(BaseAgent):
    def __init__(self):
        super().__init__(agent_type="FrontendAgent")

        # LLM Chain for generating frontend code
        self.code_gen_chain = LLMChain(
            llm=self.llm,
            prompt=ChatPromptTemplate.from_template(FRONTEND_CODE_GEN_PROMPT)
        )
    ...
    def _generate_code_and_create_pr(self, ticket: dict):
        ...
        try:
            ...
            generation = json.loads(generation_str)
